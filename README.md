...
The Pixel Block is a set of at least 10 pixels, each of which redirects to a separate partner, passing them the recipient's hashed email address and allowing the partner to set a cookie.
...
| `emailMd5Lower` | MD5 hash of lowercased email address. | No |
| `emailSha1Lower` | SHA1 hash of lowercased email address. | No |
| `emailSha256Lower` | SHA256 hash of lowercased email address. | No |
| `domain` | The recipient's domain. | No |

__Note__: At least one of the forms of hashes is required. Some partners expect an MD5 hash, but others expect SHA-1 or SHA-256. In order to maximize revenue, please provide all: `emailMd5Lower`, `emailSha1Lower`, or `emailSha256Lower`. It is important that you use the same hash parameters with each pixel.
...
Include at least 10 pixels with the same hashes provided. You can include one or more of `emailMd5Lower`, `emailSha1Lower`, and `emailSha256Lower`, but it is important that you use the same hash parameters with each pixel.

Like so (*notice the changing `index`*):
